...
module.exports = {
  ...
  async execute(message, arguments2, fsh) {
    let data = await fetch(`https://api.urbandictionary.com/v0/define?term=${arguments2[0]}`)
    data = await data.json();
    data = data.list.filter(e => {return e.word == arguments2[0]}).sort((a,b) => {return a.thumbs_up > b.thumbs_up ? -1 : 1});
    data = data[0]
