...
${shuffle(["- Text - Random letters (a-z,A-Z)","- String - Random letters and numbers (a-z,A-Z,0-9)","- Number - Random number (0-9)","- Fraction - 0. numbers (0-9)","- Member - Random users on the server"]).join("\n")}`)
        break;
      case 'text':
...
\`\`\``)
      ...
      case 'member':
        ...
        for (i = 0; i < (arguments2[1]||1); i++) {
          ...
          y.push(`<@${user.user.id}> - ${user.displayName} (${user.user.id})`)
        }
